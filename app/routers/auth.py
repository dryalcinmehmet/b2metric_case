...
from app.core.jwt import (
    ...
    decode_access_token_for_verify,
    oauth2_scheme,
    SUB,
    JTI,
    EXP,
...
async def register(
    ...
    if user:
        raise HTTPException(
            status_code=400, detail="Username is not available, please try a new one."
        )
...
async def verify(token: str, db: DBSessionDep):
    payload = await decode_access_token_for_verify(token=token, db=db)
...
async def forgot_password(
    ...
    if not user:
        return {
            "msg": "Email is not regestered in our database. Please check the email or register for a new account"
        }
    else:
        user_schema = UserSchema.model_validate(user.__dict__)
...
async def password_update(
    ...
    hashed_password = hash_password(data.password)
    await user.patch(db=db, username=payload.username, password=hashed_password)
